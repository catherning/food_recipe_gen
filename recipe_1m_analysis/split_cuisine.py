...
def createZeroShotDataset(folder):
    for file in FILE:
      with open(os.path.join(folder,"recipe1m_{}_cuisine_nn.pkl".format(file)),'rb') as f:
        data=pickle.load(f)
        
      for k,sp in split.items():
        new_data = dict(filter(lambda rec: rec[1]["cuisine"] in sp,data.items()))
        with open(os.path.join(folder,os.pardir, 'recipe1m_{}_nn_{}.pkl'.format(file,k)), 'wb') as f:
            pickle.dump(new_data, f)
